"""
Cross-family validation: Run key experiments with Mistral-7B-Instruct-v0.3
to verify that Relevance Dominance and Diversity Blindness are not
Qwen-specific artifacts.
"""
...
MISTRAL_PATH = "/root/models/modelscope_cache/LLM-Research/Mistral-7B-Instruct-v0___3"
...
def main():
    out_dir = RESULTS_DIR / "mistral"
    out_dir.mkdir(parents=True, exist_ok=True)

    log(f"Loading Mistral-7B-Instruct: {MISTRAL_PATH}")
    config = JudgeConfig(
        ...
        for model_a, model_b in pairs:
            ...
            for dim in VALIDATION_DIMS:
                ...
                try:
                    ...
                    fname = out_dir / f"{ds}_{model_a}_vs_{model_b}_{dim}_mistral.json"
    ...
    with open(out_dir / "all_results_mistral.json", "w") as f:
        json.dump(all_results, f, indent=2, default=str)

    elapsed = time.time() - start
    log(f"\nMistral validation complete! {elapsed/60:.1f} min")
    judge.cleanup()
